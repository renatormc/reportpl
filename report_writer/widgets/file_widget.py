...
class FileWidget:
    ...
    def get_update_data(self, payload: Any) -> Any:
        if self.file_parser is None:
            return {}
        path = self.form.report_writer.get_widget_assets_folder(self.name, create=True) / str(payload['relpath'])
        if path.exists():
    ...
    def convert_data(self, raw_data: Any) -> Tuple[Any, ErrorsType]:
        ...
        data = self.form.report_writer.get_widget_assets_folder(self.name, create=True) / text
        print(data)
        if not data.exists():
            return None, "Campo obrigatório"
        try:
